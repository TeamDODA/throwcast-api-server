...
const assignStations = (channels) => {
  podcasts.map(podcast => {
    for (let i = 0; i < channels.length; i++) {
      if (channels[i].imageUrl === podcast.imageUrl) {
        podcast.station = channels[i]._id;
        break;
      }
    }
  });
...
const seed = () => {
  Station.create(stations)
    .then(createdStations => assignStations(createdStations))
    .then(() => Podcast.create(podcasts))
    .then(result => console.log(result))
    .catch(err => console.log(err));
};

db.connect().then(() => seed());
